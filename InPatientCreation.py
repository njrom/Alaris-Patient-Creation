...
# TODO: When switching to tst change grand central label


def main():
    patients = Patients(1)  # Number determines the name
    patients.addPatients(1)  # Number specifies how many patients you would like
    login()
    # --------------------------------------
...
def addPCP(patient):
    ...
    click("Accept.png")
    wait(2)
    click("GrandCentralLabelTST.png")
    wait(1)
...
def addEncounterInfo(patient):
    ...
    if patient["room"] != "n/a":
        type(patient["room"] + Key.TAB*3)

    else:
        type(Key.TAB * 3)
    type(patient["Accommodation code"]+Key.TAB*3)
