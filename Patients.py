...
class Patients:
    ...
    def addPatients(self, numOfPatients): # Reads in the csv of the data table from excel
        # Change This here
        PROJECT_NAME = "AlarisReserved"
        PATIENT_TYPE = "ADULTHH"

        file = open(PATIENT_TYPE+".csv", 'r')
        ...
        for line in lines[1:]:
            ...
            if patient["name"] == "n/a":
                patient["name"] = PROJECT_NAME +","+ PATIENT_TYPE+self.int_to_en(self.PATIENT_NUM)
            self.PATIENT_NUM += 1
            counter += 1
